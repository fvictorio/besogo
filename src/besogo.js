...
const { addKeypressHandler, addWheelHandler } = require('./dom')
...
// create board in element `container` using `options`
besogo.create = function (container, options) {
  var editor, // Core editor object
    ...
    i,
    panelName // Scratch iteration variables
  ...
  options.size = parseSize(options.size || 19)
  ...
  function setDimensions(width, height) {
    if (height && width > height) {
      ...
      // Portrait mode (implied if height is missing)
      container.style['flex-direction'] = 'column'
      boardDiv.style.height = width + 'px'
      boardDiv.style.width = width + 'px'
      if (panelsDiv) {
        if (height) {
          // Only set height if param present
...
function navigatePath(editor, path) {
  var subPaths, i, j // Scratch iteration variables
...
besogo.makeBoardDisplay = function (container, editor) {
  ...
  function initializeBoard(coord) {
    ...
    if (editor.REAL_STONES) {
      // Generate index for realistic stone images
      randomizeIndex()
    }
  }
  ...
  function drawBoard(coord) {
    ...
    for (i = 2; i <= sizeY - 1; i++) {
      ...
      string +=
        'M' + svgPos(1) + ',' + svgPos(i) + 'h' + CELL_SIZE * (sizeX - 1)
    }
    for (i = 2; i <= sizeX - 1; i++) {
      // Vertical inner lines
      string +=
        'M' + svgPos(i) + ',' + svgPos(1) + 'v' + CELL_SIZE * (sizeY - 1)
    }
    svg.appendChild(
      besogo.svgEl('path', {
  ...
  function drawCoords(coord) {
    var labels = Coord[coord](sizeX, sizeY),
      ...
      i,
      x,
      y // Scratch iteration variable

    for (i = 1; i <= sizeX; i++) {
      // Draw column coordinate labels
      x = svgPos(i)
  ...
  function drawHoshi() {
    ...
    if (pathStr) {
      // Only need to add if hoshi drawn
      svg.appendChild(
        besogo.svgEl('path', {
          // Drawing circles via path points
    ...
    function drawStar(i, j) {
      // Extend path string to draw star point
    ...
    for (i = 1; i <= sizeX; i++) {
      for (j = 1; j <= sizeY; j++) {
        element = besogo.svgEl('rect', {
          // Make a transparent event target
        ...
        if (!TOUCH_FLAG) {
          // Skip hover listeners for touch interfaces
  ...
  function handleClick(i, j) {
    // Returns function for click handling
  ...
  function handleOver(i, j) {
    // Returns function for mouse over
    return function () {
      var element = hoverLayer[fromXY(i, j)]
      if (element) {
        // Make tool action visible on hover over
        element.setAttribute('visibility', 'visible')
      }
    }
  }
  function handleOut(i, j) {
    // Returns function for mouse off
    return function () {
      var element = hoverLayer[fromXY(i, j)]
      if (element) {
        // Make tool action invisible on hover off
  ...
  function redrawStones(current) {
    var group = besogo.svgEl('g'), // New stone layer group
      ...
      x,
      y,
      color // Scratch iteration variables
      ...
      for (j = 1; j <= sizeY; j++) {
        ...
        if (color) {
          ...
          if (editor.REAL_STONES) {
            // Realistic stone
            group.appendChild(
              besogo.realStone(x, y, color, randIndex[fromXY(i, j)]),
            )
          } else {
            // SVG stone
      ...
      for (j = 1; j <= sizeY; j++) {
        ...
        if (mark) {
          ...
          if (typeof mark === 'number') {
            ...
            // Markup is a label
            if (!stone) {
              // If placing label on empty spot
  ...
  function redrawHover(current) {
    ...
    if (tool === 'navOnly') {
      ...
      for (i = 0; i < children.length; i++) {
        ...
        if (stone && stone.x !== 0) {
          // Child node is move and not a pass
          x = svgPos(stone.x)
          y = svgPos(stone.y)
      ...
      // Render hover for other tools by iterating over grid
      for (i = 1; i <= sizeX; i++) {
        for (j = 1; j <= sizeY; j++) {
          element = null
          x = svgPos(i)
...
besogo.makeCommentPanel = function (container, editor) {
  ...
  function update(msg) {
    ...
    if (msg.navChange) {
      ...
      if (
        editor.getCurrent() === editor.getRoot() &&
  ...
  function updateGameInfoTable(gameInfo) {
    var table = document.createElement('table'),
      ...
      row,
      cell,
      text // Scratch iteration variable
    ...
    for (i = 0; i < infoOrder.length; i++) {
      ...
      if (gameInfo[id]) {
        // Only add row if property exists
...
besogo.makeControlPanel = function (container, editor) {
  ...
  function update(msg) {
    ...
    function updateStyleButtons(style) {
      // Updates the variant style buttons
      if (style % 2) {
        // Sibling style variants
    ...
    function arraySetColor(list, color) {
      // Changes fill color of list of svg elements
      var i
  ...
  function drawNavButtons() {
    ...
    leftElements.push(
      makeNavButton('Previous node', '85,10 85,90 15,50', function () {
        editor.prevNode(1)
      }),
    )

    rightElements.push(
      makeNavButton('Next node', '15,10 15,90 85,50', function () {
        editor.nextNode(1)
      }),
    )
    rightElements.push(
    ...
    siblingElements.push(
      makeNavButton('Previous sibling', '10,85 90,85 50,15', function () {
        editor.nextSibling(-1)
      }),
    )
    siblingElements.push(
      makeNavButton('Next sibling', '10,15 90,15 50,85', function () {
        editor.nextSibling(1)
      }),
    )
...
besogo.makeEditor = function (sizeX, sizeY) {
  ...
  function toggleVariantStyle(toggleShow) {
    ...
    if (toggleShow) {
      // Truthy input toggles showing of auto-markup
      showStyle = (showStyle + 2) % 4 // 0 => 2 or 2 => 0
    } else {
      // Falsy input toggles child vs sibling style
...
besogo.makeFilePanel = function (container, editor) {
  ...
  element.onclick = function () {
    // Bind click to the hidden file chooser
    fileChooser.click()
  }
  ...
  function readFile(evt) {
    ...
    reader.onload = function (e) {
      // Parse and load game tree
      var sgf
      try {
        sgf = parseSgf(e.target.result)
      } catch (error) {
...
besogo.makeGameRoot = function (sizeX, sizeY) {
  ...
  var BLACK = -1, // Stone state constants
    ...
    root = {
      // Inherited attributes of root node
      blackCaps: 0,
      whiteCaps: 0,
      moveNumber: 0,
    }
  ...
  root.playMove = function (x, y, color, allow) {
    ...
    if (!color) {
      // Falsy color indicates auto-color
      color = this.nextMove()
    }
    ...
    if (
      !allow &&
      prevMove && // If previous move exists, ...
    ...
    if (captures === 0) {
      // Check for suicide if nothing was captured
    ...
    if (color * captures < 0) {
      // Capture by black or suicide by white
      this.blackCaps += Math.abs(captures) // Tally captures for black
    } else {
      // Capture by white or suicide by black
  ...
  root.addMarkup = function (x, y, mark) {
    ...
    if (this.getMarkup(x, y) === mark) {
      // Quit early if no change to make
      return false
    }
  ...
  root.getSetup = function (x, y) {
    if (!this.setupStones[fromXY(x, y)]) {
      // No setup stone placed
      return false
    } else {
      // Determine net effect of setup stone
  ...
  root.getType = function () {
    ...
    if (this.move) {
      // Logged move implies move node
      return 'move'
    }

    for (i = 0; i < this.setupStones.length; i++) {
      if (this.setupStones[i]) {
        // Any setup stones implies setup node
        return 'setup'
      }
    }
  ...
  root.promote = function (child) {
    var i = this.children.indexOf(child)
    if (i > 0) {
      // Child exists and not already first
  ...
  root.demote = function (child) {
    ...
    if (i !== -1 && i < this.children.length - 1) {
      // Child exists and not already last
      this.children[i] = this.children[i + 1]
      this.children[i + 1] = child
      return true
    }
...
besogo.loadSgf = function (sgf, editor) {
      ...
      if (param === 'label') {
        ...
        if (values[i].charAt(2) === ':') {
          ...
          if (otherPoint.x === point.x && otherPoint.y === point.y) {
            ...
            // Iterate over the compressed points
  ...
  function loadRootProps(node) {
    var gameInfoIds = [
      ...
      i,
      id,
      value // Scratch iteration variables
    ...
    for (i = 0; i < node.props.length; i++) {
      ...
      if (id === 'SZ') {
        ...
        // Game info property
        if (id !== 'GC') {
          // Treat all but GC as simpletext
          value = value.replace(/\n/g, ' ') // Convert line breaks to spaces
        }
        if (value) {
          // Skip load of empty game info strings
          gameInfo[id] = value
        }
      }
    }
  ...
  function lettersToCoords(letters) {
    if (letters.match(/^[A-Za-z]{2}$/)) {
      // Verify input is two letters
      return {
        x: charToNum(letters.charAt(0)),
        y: charToNum(letters.charAt(1)),
      }
    } else {
      // Anything but two letters
      return { x: 0, y: 0 } // Return (0, 0) coordinates
    }
  }

  function charToNum(c) {
    // Helper for lettersToCoords
    if (c.match(/[A-Z]/)) {
      // Letters A-Z to 27-52
...
besogo.makeNamesPanel = function (container, editor) {
  ...
  function update(msg) {
    ...
    if (msg.gameInfo) {
      infoString =
        (msg.gameInfo.PW || 'White') + // White name
        ' (' +
        (msg.gameInfo.WR || '?') +
        ')' + // White rank
      ...
      infoString =
        (msg.gameInfo.PB || 'Black') + // Black name
        ' (' +
        (msg.gameInfo.BR || '?') +
        ')' + // Black rank
    ...
    if (msg.navChange || msg.stoneChange) {
      ...
      updateText(
        whiteCaps,
        (passFlag === 1 ? 'Passed  ' : '') + current.whiteCaps,
      )
      updateText(
        blackCaps,
        current.blackCaps + (passFlag === -1 ? '  Passed' : ''),
      )
    }
  }
...
besogo.composeSgf = function (editor) {
  ...
  function composeNode(tree) {
    ...
    if (!tree.parent) {
      // Null parent means node is root
    ...
    if (children.length === 1) {
      // Continue sequence if only one child
  ...
  function composeRootProps(tree) {
    ...
    for (id in gameInfo) {
      // Compose game info properties
      if (Object.prototype.hasOwnProperty.call(gameInfo, id) && gameInfo[id]) {
        // Skip empty strings
  ...
  function composePointLists(lists) {
    ...
    for (id in lists) {
      // Object own keys specifies property IDs
      if (Object.prototype.hasOwnProperty.call(lists, id)) {
        points = lists[id] // Corresponding members are point lists
        if (points.length > 0) {
          // Only add property if list non-empty
          string += id
          for (i = 0; i < points.length; i++) {
            string += '[' + coordsToLetters(points[i].x, points[i].y)
            if (points[i].label) {
              // Add optional composed label
  ...
  function numToChar(num) {
    // Helper for coordsToLetters
    if (num > 26) {
      // Numbers 27-52 to A-Z
  ...
  function toolStateUpdate(msg) {
    ...
    if (msg.tool) {
      for (tool in selectors) {
        ...
        if (Object.prototype.hasOwnProperty.call(selectors, tool)) {
          if (msg.tool === tool) {
...
besogo.makeTreePanel = function (container, editor) {
  ...
  function treeUpdate(msg) {
    if (msg.treeChange) {
      ...
      // Only stones in current changed
  ...
  function rebuildNavTree() {
    ...
    if (oldSvg) {
      // Replace SVG in container
      container.replaceChild(svg, oldSvg)
    } else {
      // SVG not yet added to container
  ...
  function makeNodeIcon(node, x, y) {
    // Makes a node icon for the tree
    var element, color
