...
Coord.western = function (sizeX, sizeY) {
  var labels = { x: [], y: [] },
    i
  for (i = 1; i <= sizeX; i++) {
...
Coord.numeric = function (sizeX, sizeY) {
  var labels = { x: [], y: [] },
    i
  for (i = 1; i <= sizeX; i++) {
    labels.x[i] = i + ''
...
Coord.pierre = function (sizeX, sizeY) {
  var labels = { x: [], xb: [], y: [], yb: [] },
    i
  for (i = 1; i <= sizeX / 2; i++) {
    labels.x[i] = 'a' + i
...
Coord.corner = function (sizeX, sizeY) {
  var labels = { x: [], y: [] },
    i
  for (i = 1; i <= sizeX; i++) {
    if (i < sizeX / 2 + 1) {
...
Coord.eastcor = function (sizeX, sizeY) {
  var labels = { x: [], y: [] },
    i
  for (i = 1; i <= sizeX; i++) {
    if (i < sizeX / 2 + 1) {
...
Coord.eastern = function (sizeX, sizeY) {
  var labels = { x: [], y: [] },
    i
  for (i = 1; i <= sizeX; i++) {
