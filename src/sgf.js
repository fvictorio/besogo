module.exports.parseSgf = function (text) {
  ...
  function next(check) {
    if (check && check !== ch) {
      // Verify current character if param given
  ...
  function lineBreak() {
    if (ch === '\n') {
      ...
      if (text.charAt(at + 1) === '\r') {
        // LF+CR, double character line break
  ...
  function parseTree() {
    ...
    if (ch !== ';') {
      // Error on sub-tree missing root node
    ...
    while (ch === ';') {
      // Get sequence of nodes within this sub-tree
